...
/*$mysql="SELECT MIN(`create_on_date`) AS a, MAX(`create_on_date`) AS b FROM `wifi_live_plans`";
...
$mainsql="SELECT  `uid`,`start_time`,`end_time`,`create_by_user`, `user_access_plan_name`, `verify_type`, `verify_info`, `plan_price` FROM ( SELECT  a.`uid`,a.`start_time`,a.`end_time`,a.`create_by_user`,c.`user_access_plan_name`,a.`verify_type`,a.`verify_info`,a.`plan_price` FROM `wifi_live_plans` as a  ,`mac_user_info` as b,`wifi_access_plan` as c WHERE a.`mac_uid` = b.`uid` AND  a.`plan_uid`= c.`uid` AND b.`uid` = '".$getuid."' AND DATE(a.`create_on_date`) >= '".$startdate."' AND DATE(a.`create_on_date`) <= '".$enddate."' ORDER BY a.uid DESC) z WHERE 1 ";
}
...
if($order[$e]['column'] == $xaa){$orderbycol=$orderbycollast;}$orderbycollast=$xaa;$z++;}
if($orderbycol!=""){
