...
$mainsql="SELECT  `uid`, `value1`,`value2`, `value3`,`user_access_plan_name`, `no_of_devices_allowed`, `package_active`,countx FROM ( 

SELECT a.`uid`, a.`value1`,a.`value2`, a.`value3`,a.`user_access_plan_name`, a.`no_of_devices_allowed`, a.`package_active`, 
(CASE WHEN b.`countx`  IS NULL THEN 0 ELSE b.`countx` END) as countx
 FROM 
(
SELECT a.`uid`, a.`value1`,a.`value2`, a.`value3`,b.`user_access_plan_name`, a.`no_of_devices_allowed`, a.`package_active`  FROM `wifi_package_management` as a  ,`wifi_access_plan` as b WHERE a.`access_plan_id`= b.`uid` 
) a 
left join
(
SELECT p.package_uid,count(DISTINCT u.uid) as countx FROM  wifi_live_plans as p LEFT JOIN mac_user_info as u  ON   u.uid=p.mac_uid group by p.package_uid
) b 
ON b.package_uid=a.uid
";
...
if($j==6 && $mysqlrow[$j]=="0"){$mysqlrow[$j]="InActive";}
if($j==6 && $mysqlrow[$j]=="1"){$mysqlrow[$j]="Active";}
if($j==7 && $mysqlrow[$j]=="0"){$mysqlrow[$j]="None";}
