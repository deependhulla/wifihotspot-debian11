...
#$mainsql="SELECT `uid`, `user_full_name`,`user_mobile`, `user_email`, `user_mac_address`,  `user_reg_datetime`, `user_reg_active`,`user_mac_blocked`, `user_mobile_blocked`,`package_uid` FROM (SELECT u.uid,u.user_full_name,u.user_mobile,u.user_email,u.user_mac_address,u.user_reg_datetime,u.user_reg_active,u.user_mac_blocked,u.user_mobile_blocked,c.package_uid FROM mac_user_info as u, wifi_live_plans as c WHERE u.uid=c.mac_uid)z where 1";

$mainsql="SELECT `uid`, `user_full_name`,`user_mobile`, `user_email`, `user_mac_address`, `user_reg_datetime`, `user_reg_active`,`user_mac_blocked`, `user_mobile_blocked`, `package_uid` FROM (SELECT u.uid,u.user_full_name,u.user_mobile,u.user_email,u.user_mac_address,u.user_reg_datetime,u.user_reg_active,u.user_mac_blocked,u.user_mobile_blocked, p.package_uid FROM mac_user_info as u LEFT JOIN wifi_live_plans as p ON u.uid=p.mac_uid AND NOT EXISTS (SELECT 1 FROM wifi_live_plans p1 WHERE p1.mac_uid = u.uid AND p1.uid > p.uid ))z where 1 ";
...
if($j==9 && $mysqlrow[$j]=="0"){$mysqlrow[$j]="None";}
print "\"".$mysqlrow[$j]."\"";
}
